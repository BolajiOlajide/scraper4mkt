...
func CsvOutput(data top.GithubDataPieces, writer io.Writer) error {
	...
	if err := w.Write([]string{"rank", "name", "login", "contributions", "company", "organizations", "email"}); err != nil {
		return err
	}
	for i, piece := range data {
		...
		if err := w.Write([]string{rank, name, login, contribs, company, orgs, email}); err != nil {
			return err
		}
	}
	w.Flush()
